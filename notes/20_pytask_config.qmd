...
inputs = list(data_catalog['download']._entries)
inputs.remove("queries")  # remove the "queries" entry
...
```{python}
#| export
inp_out = pd.DataFrame({"input": inputs, "output": outputs})
aggregate_jobs = inp_out.merge(agg_params, how="cross")
```

This result gives us the full list of jobs for the aggregation task. 26 rows for the parameters,
and 386 inputs/outputs, giving a total of 10036 jobs:
