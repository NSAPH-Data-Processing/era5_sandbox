...
def task_aggregate_data_diurnal(
        input_file: Path, # input data Path from the download task
        ...
        time: Literal["day", "night"] = "day", # whether to aggregate by day or night
        night_direction: Literal["before", "after"] = "before", # how to define night
        variable: str = "2m_dewpoint_temperature", # the variable to aggregate
    ) -> None:
    """
    Aggregate data from a CDSAPI Query to the healthshed
    level. 
    """
    jobs = data_catalog['aggregate']['jobs'].load()

    for i, row in jobs.iterrows():

        if row['variables'] != variable:
            continue
        if row['time'] != time:
            continue
        if row['aggregation_name'] != aggregation_name:
            continue

        print(f"Processing job {i+1}/{len(jobs)}: variable={variable}, time={time}, aggregation={aggregation_name}")
        ...
        if time == "day":
            ds_masked = ds.copy()
            ex=compute_diurnal_class_bins(ds_masked)
            ds_masked['solar_class'] = (('valid_time', 'latitude', 'longitude'), ex)
            ds_time = ds_masked.where(ds_masked["solar_class"] == "day").drop_vars(["solar_class"])
        elif time == "night":
            ds_masked = ds.copy()
            ex, ex_dt = compute_solar_day_night_class_bins(ds_masked, night_direction)
            ds_masked['solar_class'] = (('valid_time', 'latitude', 'longitude'), ex)
            ds_masked["solar_date"] = (("valid_time", "latitude", "longitude"), ex_dt)
            ds_time = ds_masked.where(ds_masked["solar_class"] == "night").drop_vars(["solar_class", "solar_date"])
        else:
            raise ValueError(f"Invalid time: {time}")

        # set the local timezone for Madagascar
        ds_time = ds_time.assign_coords(valid_time=pd.to_datetime(ds["valid_time"].values).tz_localize("UTC").tz_convert("Indian/Antananarivo"))

        # resample by day or night
        ds_time_rs = ds_time.resample(valid_time="1D").reduce(np.nanmean)

        # convert to tiff
        raster = netcdf_to_tiff(ds_time_rs, band=1, variable=variable)
