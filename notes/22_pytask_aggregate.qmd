...
Now, to see if it will resample by both solar day and diurnal class. This was a little complicated,
so ChatGPT recommended to first "stack" the dataset so that the z dimension is flattened to a multi-index dataframe:
...
Then, by treating it as a dataframe, statistical operations are easy:
...
Then, you can unstack the dataframe to return it to an `xarray`:
...
```{python}
agg_func = np.mean
vars_to_aggregate = [v for v in ds.data_vars if v not in ["solar_date", "solar_class"]]
agg_vars = []

for var in vars_to_aggregate:
    df = ds_stacked[[var, "solar_date", "solar_class"]].to_dataframe().dropna()
    df = df.drop(columns=["latitude", "longitude"])
    grouped = df.groupby(["solar_date", "solar_class", "latitude", "longitude"])[var]
...
for date in ds_resampled.solar_date.values:
    ds_day = ds_resampled.sel(solar_class="day", solar_date=date).drop_vars("solar_class")
    ds_day = ds_day.rename({"solar_date": "valid_time"})
    raster = netcdf_to_tiff(ds_day, band=1, variable="swvl1")
    print(raster)
...
```{python}
resampled_tiff_day.data.shape, resampled_tiff_day.transform, resampled_tiff_day.crs, resampled_tiff_day.bounds
```
